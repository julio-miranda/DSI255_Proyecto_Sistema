...
public class ReservasProvider {
    ...
    public static void cargarTabla(JTable table) {
        SimpleDateFormat sdfOriginal = new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.ENGLISH);
        ...
        try {
            CollectionReference personas = conexion.db.collection("reservas");
            ...
            for (DocumentSnapshot document : querySnap.get().getDocuments()) {
                ...
                if (esFormatoOriginal(document.getString("fechainicio")) && esFormatoOriginal(document.getString("fechafinal"))) {
                    model.addRow(obtenerFilaConFechasConvertidas(document, sdfOriginal, sdfDeseado));
                } else {
                    model.addRow(obtenerFilaSinConversionDeFechas(document));
                }
            }
    ...
    public static boolean esFormatoOriginal(String fecha) {
        SimpleDateFormat sdfOriginal = new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.ENGLISH);
        try {
            sdfOriginal.parse(fecha);
            return true;
    ...
    // Método para obtener una fila con fechas convertidas
    private static Object[] obtenerFilaConFechasConvertidas(DocumentSnapshot document, SimpleDateFormat sdfOriginal, SimpleDateFormat sdfDeseado) throws ParseException {
        Date dateI = sdfOriginal.parse(document.getString("fechainicio"));
        Date dateF = sdfOriginal.parse(document.getString("fechafinal"));

        String fechaDeseadaI = sdfDeseado.format(dateI);
        String fechaDeseadaF = sdfDeseado.format(dateF);

        String codigo = obtenerCodigoBungalow(document.getString("Bungalow"));

        agregarFechasOcupadas(sdfDeseado.parse(fechaDeseadaI), sdfDeseado.parse(fechaDeseadaF), codigo);
...
// Método para obtener una fila sin conversión de fechas
    private static Object[] obtenerFilaSinConversionDeFechas(DocumentSnapshot document) {
